...
class NagaSkill(MycroftSkill):
    SEC_PER_LETTER = 0.65  # timing based on Mark 1 screen
    LETTERS_PER_SCREEN = 9.0

    def __init__(self):
        super(NagaSkill, self).__init__(name="NagaSkill")

    @intent_handler(IntentBuilder("NagaSkill").require("query").require("naga"))
    def handle_query_IP(self, message):
        ...
        if len(addr) == 0:
            ...
            for iface in addr:
                ...
                self.enclosure.mouth_text(ip)
                ip_spoken = ip.replace(".", " "+dot+" ")
                self.speak_dialog("my Naga on X is Y",
